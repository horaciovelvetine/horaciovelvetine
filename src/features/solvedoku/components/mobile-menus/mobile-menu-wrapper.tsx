...
import type { PuzzleDifficulty, SiteSettings, SolvedokuGameState } from '../../../../types';
...
export function MobileMenuWrapper({
  ...
  return (
    <div
      className={`absolute w-full -bottom-2 transition-transform duration-200 ${showMobileMenu ? '' : 'translate-y-250'}`}>
      <div className='flex flex-col bg-stone-900 text-white px-2 pt-2 items-center gap-1'>
        <div className='flex w-full justify-end'>
          <img
            src={CloseIcon}
            className='size-6 xs:size-8 sm:size-10'
            onClick={() => {
        ...
        {/* ADD MAIN CONTENT */}
        <Content setShowMobileMenu={setShowMobileMenu} solvedokuState={solvedokuState} siteSettings={siteSettings} setCurrentPuzzleDifficultyDisplay={setCurrentPuzzleDifficultyDisplay} />
      </div>
    </div>
  );
}
